...
function loadGoals() {
  try {
    const raw = localStorage.getItem("finflow_goals");
...
function saveGoals(data: Goal[]) {
  localStorage.setItem("finflow_goals", JSON.stringify(data));
}
...
function GoalCard({ goal, onUpdate, onDelete }: { goal: Goal; onUpdate: (g: Goal) => void; onDelete: (id: string) => void }) {
  ...
  const handleAddFunds = () => {
    ...
    if (!amt || amt <= 0) return;
    onUpdate({ ...goal, current: Math.min(goal.current + amt, goal.target) });
    setEditAmount("");
    setEditMode(false);
  };
    ...
    <div className="glass rounded-2xl p-5 border border-white/[0.04] group">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-white/25">{new Date(goal.deadline).toLocaleDateString("ru-RU")}</p>
          </div>
        </div>
      ...
      <div className="mb-2">
        <div className="flex justify-between text-xs mb-1">
          <span className="text-white/40">{goal.current.toLocaleString("ru-RU")} ₽</span>
          <span className="text-white/25">{goal.target.toLocaleString("ru-RU")} ₽</span>
        </div>
...
export default function GoalsPage() {
  ...
  const deleteGoal = useCallback(() => {
    ...
    const next = goals.filter((g) => g.id !== deleteConfirm);
    setGoals(next);
    saveGoals(next);
