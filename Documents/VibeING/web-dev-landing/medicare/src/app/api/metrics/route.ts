/**
 * Metrics API — MediCare (3002)
 * 🔒 Fixed: Full implementation with input validation.
 * Uses in-memory storage with fallback to MongoDB (Prisma) when connected.
 */
...
// In-memory storage (will use MongoDB in production when DB is connected)
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    // Store in memory (production: use Prisma/MongoDB)
